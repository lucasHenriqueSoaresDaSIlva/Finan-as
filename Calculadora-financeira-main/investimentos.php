<?php
// Função para calcular investimento com juros compostos
function calcularInvestimento($valor, $taxa_aa, $prazo) {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($valor <= 0 || $prazo < 12 || $taxa_aa < 0) {
        ...
        $result = calcularInvestimento($valor, $taxa_aa, $prazo);
    }
}
?>
<!DOCTYPE html>
    ...
    <div class="simulator-container">
        ...
        <form class="simulator-form" method="post" autocomplete="off">
            ...
            <select name="tipo" id="tipo">
                <option <?=(!isset($_POST["tipo"])||$_POST["tipo"]=="Poupança")?'selected':''?>>Poupança</option>
                <option <?=(@$_POST["tipo"]=="CDB")?'selected':''?>>CDB</option>
                <option <?=(@$_POST["tipo"]=="Tesouro Direto")?'selected':''?>>Tesouro Direto</option>
                <option <?=(@$_POST["tipo"]=="Fundos")?'selected':''?>>Fundos</option>
            </select>

            <label for="valor">Aporte Inicial (R$):</label>
            <input type="number" min="0.01" name="valor" id="valor" step="0.01" 
                   value="<?=isset($_POST["valor"])?htmlspecialchars($_POST["valor"]):""?>" required>

            <label for="prazo" class="slider-label">Prazo (Meses):</label>
            <input type="range" name="prazo" id="prazo" min="12" max="240" 
                   value="<?=isset($_POST["prazo"])?$_POST["prazo"]:60?>" 
                   oninput="prazo_out.value=this.value">
            <output id="prazo_out"><?=isset($_POST["prazo"])?$_POST["prazo"]:60?></output> meses

            <label for="taxa">Rentabilidade (% a.a.):</label>
            <input type="number" min="0" name="taxa" id="taxa" step="0.01" 
                   value="<?=isset($_POST["taxa"])?htmlspecialchars($_POST["taxa"]):"8.00"?>" required>

            <button type="submit"><i class="fa fa-chart-line"></i> Simular</button>
        </form>
        ...
        <div class="simulation-result">
            ...
            <div>Montante Final: <strong>R$ <?=number_format($result["montante_final"],2,",",".")?></strong></div>
            <div>Juros Ganhos: <strong>R$ <?=number_format($result["juros_ganhos"],2,",",".")?></strong></div>

            <div class="chart-container" style="margin-top:25px;">
                <canvas id="graficoInvestimento" height="160"></canvas>
            </div>
            <script>
            // Gráfico do crescimento do investimento
            ...
            const chartData = {
                ...
                datasets: [{
                    ...
                    borderColor: '#006600',
                    backgroundColor: 'rgba(0,102,0,0.08)',
                    fill:true,
                    tension:0.15,
                    pointRadius:2
                }]
